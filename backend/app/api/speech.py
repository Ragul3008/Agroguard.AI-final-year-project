...
class SynthesisRequest(BaseModel):
    text:     str = Field(..., example="வாழை இலையில் மஞ்சள் புள்ளிகள்")
    language: str = Field(default="en", example="ta")
...
@router.post(
    "/speech/synthesize",
    summary="Convert text to audio speech (TTS)",
    description="Synthesizes text into MP3 audio in requested language (ta, hi, te, kn, ml, en, mr, etc.).",
)
async def synthesize_speech(request: SynthesisRequest):
    logger.info("POST /speech/synthesize | lang='%s' text_len=%d", request.language, len(request.text))
    try:
        audio_bytes = _speech_service.synthesize_speech(
            text=request.text,
        ...
        raise HTTPException(status_code=500, detail="Failed to generate audio speech.")